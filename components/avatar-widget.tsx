...
// Longest edge a compressed receipt photo is allowed to keep — a ticket de
// caisse is legible to the AI well below this, and shrinking upstream (here,
// in the browser) rather than raising server-side limits keeps every
// upload fast regardless of how large the source phone photo is.
const MAX_DIMENSION = 1600;
const JPEG_QUALITY = 0.8;

// Downscales + re-encodes a photo as JPEG entirely client-side (Canvas API,
// no dependency) before it ever reaches the network. A modern phone photo
// can be 15-25MB uncompressed; this routinely brings that under ~500KB,
// which is both faster to upload and faster for the vision model to
// process. Falls back to the original file if anything about the decode
// pipeline fails (e.g. an unsupported format slipping past `accept`).
      ...
      {open && (
        <div className="flex max-h-[70vh] w-[360px] max-w-[90vw] flex-col overflow-hidden rounded-xl border border-border bg-card shadow-xl">
          ...
          <div className="flex-1 space-y-3 overflow-y-auto px-4 py-3">
            ...
            {isPending && (
              <p className="text-xs text-muted-foreground">
                L&apos;assistant réfléchit… (l&apos;analyse d&apos;une photo peut prendre jusqu&apos;à 30 secondes)
              </p>
            )}
